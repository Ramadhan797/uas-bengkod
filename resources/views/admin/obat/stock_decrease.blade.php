            ...
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-2xl border border-gray-200">
                <div class="p-8">
                    <div class="text-center mb-8">
                        <h3 class="text-2xl font-bold text-gray-900 mb-2">Kurangi Stok untuk <strong>{{ $obat->nama_obat }}</strong></h3>
                        <p class="text-gray-600">Stok saat ini: <strong>{{ $obat->stok ?? 0 }}</strong></p>
                    </div>

                    <form method="POST" action="{{ route('admin.obat.decreaseStock', $obat->id) }}" class="space-y-6">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label for="jumlah" class="block text-sm font-semibold text-gray-700 mb-2">Jumlah yang akan dikurangi</label>
                        ...
                        <div class="flex gap-4 pt-6 border-t border-gray-200">
                            ...
                            <button type="submit" class="flex-1 bg-gradient-to-r from-red-600 to-red-700 text-white font-semibold py-3 px-6 rounded-xl">Kurangi Stok</button>
                        </div>
                    </form>
                </div>
            </div>
